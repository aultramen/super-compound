...
2. Cluster entries from the report's candidates: the same category or prevention rule appearing 3+ times with confidence Observed or Confirmed. Also cluster by the retro axes captured in `LRN-*` entries: no-op steering (delete to save tokens), tool economy, and information access (propose a hook, tool, or doc).
