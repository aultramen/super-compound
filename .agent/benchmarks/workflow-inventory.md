...
The v2 result stores one canonical scenario set plus counted run digests. It does
...
Antigravity; all 17 public `/sc-*` workflows; and these cross-cutting surfaces:
...
- `deterministic_estimated_tokens_v1` is a reproducible estimate, not a vendor
  billing tokenizer.
- The baseline is immutable historical evidence; do not regenerate it after an
  optimization.
- The aggregate total is a scenario workload and may count a file more than
  once. Use per-scenario results for decisions.
- A pass requires every reduction scenario to improve by more than 90%, every
  ...
  match exactly, and all smart-zone checks to remain intact.
